...
class PaintingModal extends React.Component {
    ...
    movementQuery(author) {
        ...
        let query = 'SELECT ?name ?mov WHERE { ?author rdfs:label ?name . FILTER regex(?name, "Christian Albrecht Jensen", "i") FILTER (LANG(?name) = "en") ?author dbo:movement ?movement . ?movement rdfs:label ?mov . FILTER (LANG(?mov) = "en")}';
        let self = this;
        dps.client()
            .query(query)
